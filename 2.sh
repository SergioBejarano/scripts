...
while true; do
    ...
    case $option in
        1)
            ...
            echo "$results"
            echo "Total occurrences: $(echo "$results" | wc -l)"  # Contar el número de archivos encontrados
            ;;
        2)
            read -p "Enter file path: " file
            read -p "Enter word to search: " word
            grep -n "$word" "$file" # Buscar la palabra en el archivo e imprimir con número de línea
        ...
        3)
            ...
            find "$dir" -type f -name "*$filename*" 2>/dev/null | while read -r file; do
                ...
                grep -n "$word" "$file" # Buscar la palabra dentro del archivo con número de línea
                echo "Total occurrences: $(grep -c "$word" "$file")" # Contar el número de apariciones de la palabra
            done
            ;;
        ...
        4)
            ...
            if [ -f "$file" ]; then
                echo "Total lines: $(wc -l < "$file")" # Contar las líneas en el archivo
            else
        ...
        5)
            ...
            head -n "$n" "$file" # Mostrar las primeras n líneas del archivo
            ;;
        6)
            read -p "Enter file path: " file
            read -p "Enter number of lines to display: " n
            tail -n "$n" "$file" # Mostrar las últimas n líneas del archivo
            ;;
        7)
            echo "Exiting..."
            break
